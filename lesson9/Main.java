...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        studentList.add(new Student("Евгений", Arrays.asList(courseList.get(0), courseList.get(1), courseList.get(6))));
        ...
        //2. Функция принимает на вход список Student и возвращает список из трех самых любознательных (любознательность определяется количеством курсов).
        System.out.println("\n2. Три самых любознательных студента:\n" +
                studentList.stream()
                        .sorted((s1, s2) -> {
                            if (s1.getCourses().size() == s2.getCourses().size())
                                return s1.getName().compareTo(s2.getName());
                            else if (s1.getCourses().size() < s2.getCourses().size())
                                return 1;
                            else return -1;
                        })
                        .limit(3)
